...
public class MyMap<K, V> implements Map<K, V> {
    ...
    public Set<java.util.Map.Entry<K, V>> entrySet() {
        ...
        if (keys.size() != values.size()) //jeśli nie, wyrzuć wyjątek
        {
         ...
         * Map.Entry<K, V>. Map.Entry - jest to obiekt przechowujący parę
         * klucz-wartość, czyli to co przechowują mapy. <K, V> oznacza, że dana
         * para jest typem K klucz + V wartość
         */
        Set set = new HashSet<Map.Entry<K, V>>();//todo
      //  HashMap hashMap = new HashMap();
      //  hashMap.entrySet();
        /**
         * Wrzuć wszystkie elementy mapy do TreeSetu. Dzięki temu zabiegowi,
         * elementy mapy nie będę się powtarzać i otrzymamy kolecje
         * indiwidualnych par klucz-wartość.
         */
        for (int i = 0; i < size(); i++) {
            /**
             * Nie zrażać się słowem Abstract. AbstractMap jest zwykłą klasą,
             * która potrafi trochę więcej. SimpleEntry obiektem reprezentujący
             * parę, którą można dodać do Setu.
             */
          //  set.add(( Entry<>(keys.get(i), values.get(i))));
        }
        //zwróć set
        return set;
    }
